define(['jquery', 'qlik', 'text!./template.ng.html', 'text!./dialog-template.ng.html', 'text!./dialog-template2.ng.html', 'text!./dialog-template3.ng.html', 'text!./dialog-template4.ng.html','text!./dialog-template5.ng.html', './lib/swal'],
	function ($, qlik, template, dialogTemplate, dialogTemplate2, dialogTemplate3, dialogTemplate4,dialogTemplate5) {
		'use strict';
		...
		return {
			...
			controller: ['$scope', 'luiDialog', function ($scope, luiDialog) {
				...
				$scope.openDialogVar = function () {
					luiDialog.show({
						template: dialogTemplate5,
						input: {
							name: $scope.name
						},
						controller: ['$scope', '$element', function ($scope, $element) {
									...
									.then((willDelete) => {
										if (willDelete) {
											...
											enigma.app.createSessionObject({
												...
												list.getLayout().then((layout) => {
													layout.qVariableList.qItems.forEach((element) => {
														measureArray.push(element.qInfo.qId)
													})
												}).then(() => {
													//console.log(measureArray);
													measureArray.forEach(element => {
														enigma.app.destroyVariableById(element);
													})
												});
											});
											swal({
												text: "Variables have been deleted.", 
												icon: "success",
											});
										} else {
											swal({
				...
				$scope.DestroyVariable = function(){					
					enigma.app.destroyVariableByName('Blanc1') 
					swal({
